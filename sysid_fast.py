#!/usr/bin/env python3
"""
sysid_fast.py — System Identification for ViperX-300 6-DOF Arm
================================================================
Same methodology as sysid_paper.py with performance rewrite:

  Fixes applied:
    1. DH bug fixed: d_6 = 0.0 (was L6 = 0.13658)
    2. NE backward pass index corrected: f_ext[i+1] not f_ext[i]

  Speed rewrite (regressor_fast):
    Original: 78 full NE calls per sample  (78 × FK + 78 × fwd + 78 × bwd)
    Fast:      1 FK + 1 forward pass + 1 vectorized backward pass
    Expected speedup: ~20-50× per sample

    Key idea: the NE backward pass is LINEAR in phi. Build all 78 regressor
    columns simultaneously using (3,78) numpy matrix operations instead of
    78 scalar Python loops.

  Also supports --stride N for additional subsampling (composable with fast NE).
...
# =============================================================================
# 2a. NE forward pass (shared across all 78 parameter perturbations)
# =============================================================================

def _ne_forward_pass(q, dq, ddq, T, R):
    """
    Kinematic forward pass: angular velocity/acceleration + linear acceleration
    at each link origin (gravity included via d'Alembert: ddp[0] = g).
    Returns lists of length N_JOINTS+1.
    """
...
# =============================================================================
# 2b. Vectorised NE backward pass → full (6, 78) regressor in one sweep
# =============================================================================

def regressor_fast(q, dq, ddq):
    """
    Build (6, 78) regressor W such that W @ phi = tau.

    One FK + one forward pass + one vectorized backward pass.
    Each column of W corresponds to one unit-phi perturbation; instead of
    iterating 78 times we track (3, 78) force/moment matrices and apply
    numpy bulk operations.
    """
    ...
    P = N_PARAMS_T
    # f_mat[j, k] = j-th component of force  when phi[k]=1 (all others 0), frame i
    # n_mat[j, k] = j-th component of moment when phi[k]=1
    f_mat = np.zeros((3, P))
    n_mat = np.zeros((3, P))
    ...
    for i in range(N_JOINTS, 0, -1):
        li   = i - 1              # 0-based link index
        base = li * N_PARAMS      # first phi column for this link

        ω = omega[i]; dω = domega[i]; d = ddp[i]

        # ------------------------------------------------------------------
        # Intrinsic force columns (3 × 13) for link i's parameters
        # fi = m*d + cross(dω, mc) + cross(ω, cross(ω, mc))
        # ------------------------------------------------------------------
        ...
        # params 1-3 — first mass moments mc = [mcx, mcy, mcz]
        for k, e in enumerate((EX, EY, EZ)):
            fi_cols[:, 1+k] = np.cross(dω, e) + np.cross(ω, np.cross(ω, e))

        # params 4-9 — inertia (J only enters ni, not fi)
        # params 10-12 — friction (enters tau directly, not fi/ni)

        # ------------------------------------------------------------------
        # Intrinsic moment columns (3 × 13) for link i's parameters
        # ni = J @ dω + cross(ω, J @ ω) + cross(mc, d)
        # ------------------------------------------------------------------
        ni_cols = np.zeros((3, N_PARAMS))

        # params 1-3 — mc contribution: cross(e_k, d)
        for k, e in enumerate((EX, EY, EZ)):
            ni_cols[:, 1+k] = np.cross(e, d)

        # params 4-9 — inertia basis matrices (symmetric):
        # Jxx (4): J = diag(1,0,0)
        Jdw = np.array([dω[0], 0.0,   0.0  ]); Jw = np.array([ω[0], 0.0,  0.0  ])
        ni_cols[:, 4] = Jdw + np.cross(ω, Jw)

        # Jxy (5): J = [[0,1,0],[1,0,0],[0,0,0]]
        Jdw = np.array([dω[1], dω[0], 0.0  ]); Jw = np.array([ω[1], ω[0], 0.0  ])
        ni_cols[:, 5] = Jdw + np.cross(ω, Jw)

        # Jxz (6): J = [[0,0,1],[0,0,0],[1,0,0]]
        ...
        # Place into the full (3, P) matrices
        fi_full = np.zeros((3, P)); fi_full[:, base:base+N_PARAMS] = fi_cols
        ni_full = np.zeros((3, P)); ni_full[:, base:base+N_PARAMS] = ni_cols

        # ------------------------------------------------------------------
        # Propagate wrench from next link (expressed in frame i+1 → frame i)
        # ------------------------------------------------------------------
        if i < N_JOINTS:
            Rrel_next = R[i].T @ R[i+1]          # R_{i←i+1}: frame i+1 → frame i
            r_next    = R[i].T @ (T[i+1][:3, 3] - T[i][:3, 3])   # in frame i

            f_from_next = Rrel_next @ f_mat                          # (3, P)
            # cross(r_next, f_from_next): r_next (3,), f_from_next.T (P,3) → (P,3) → .T (3,P)
        ...
        # Joint torque = z-component of net moment + friction
        W[li, :] = n_mat[2, :]
        W[li, base+10] += dq[li]               # Fv
        W[li, base+11] += np.sign(dq[li])      # Fc
        W[li, base+12] += 1.0                  # F0
...
# =============================================================================
# 2c. Scalar NE (corrected, kept for reference/validation)
# =============================================================================
...
# =============================================================================
# 5-7. Normalisation / feasibility / optimisation (unchanged)
# =============================================================================
...
def initial_phi_guess():
    ...
    for i in range(N_JOINTS):
        ...
        phi0[idx+0]  = m
        phi0[idx+4]  = m * 0.01; phi0[idx+7] = m * 0.01; phi0[idx+9] = m * 0.01
...
def run_identification(csv_path, fs=50.0, fc_lpf=10.0, stride=1,
    ...
    print("=" * 60)
    print(f"ViperX-300 SysID — Fast NE regressor (stride={stride})")
    print("=" * 60)
...
def _plot_results(t, tau_meas, tau_pred, rel):
    ...
    plt.suptitle("ViperX-300 SysID — Fast NE", fontsize=11)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="ViperX-300 sysid — fast NE version")
    ...
    parser.add_argument("--stride", type=int,   default=1,     help="Subsample stride (default 1 = all samples)")
    parser.add_argument("--no-plot", action="store_true")
    args = parser.parse_args()

    phi = run_identification(args.csv, fs=args.fs, fc_lpf=args.fc,
                             stride=args.stride, plot=not args.no_plot)
    np.save("phi_fast.npy", phi)
    print("\nSaved to phi_fast.npy")
