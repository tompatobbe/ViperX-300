...
A (84,) phi additionally carries a trailing per-joint reflected motor-inertia
block Ia (sysid_feasible --motor-inertia); Ia is reported in the summary, a
URDF comment, and the artifact sidecar — never in the <inertial> blocks.
...
def print_summary(phi, ia=None):
    ...
    if ia is not None:
        print("\nReflected motor inertia Ia [kg·m²] (per joint; feed-forward "
              "Ia·q̈, not in the URDF):")
        for i in range(N_JOINTS):
            print(f"  {JOINT_NAMES[i]:<14} {ia[i]:>12.6f}")
...
def main():
    ...
    # Load phi. (78,) = link params only; (84,) = trailing per-joint reflected
    # motor-inertia block Ia (sysid_feasible --motor-inertia). Ia cannot live in
    # a URDF <inertial> (it is local to the joint axis; a link-inertia stand-in
    # would re-introduce exactly the false cross-axis coupling it removes), so
    # it is emitted as a comment for the controller feed-forward Ia·q̈.
    phi = np.load(args.phi)
    ia = None
    if phi.shape == (84,):
        ia, phi = phi[78:], phi[:78]
    elif phi.shape != (78,):
        sys.exit(f"Error: expected phi shape (78,) or (84,), got {phi.shape}")

    if not args.no_summary:
        print(f"Loaded phi from: {args.phi}")
        print_summary(phi, ia)
    ...
    # Conditional (like sysid's flags) so pre-Ia artifacts keep their hash; the
    # values land in the sidecar JSON for the controller to read.
    if ia is not None:
        config["motor_inertia_Ia"] = {JOINT_NAMES[i]: float(ia[i])
                                      for i in range(N_JOINTS)}
    ...
    content = _build_urdf_string(phi, args, ia)
...
def _comment_ia(ia):
    """XML comment listing the reflected motor inertias (not representable in URDF)."""
    pairs = ", ".join(f"{JOINT_NAMES[i]}={ia[i]:.6f}" for i in range(N_JOINTS))
    return ET.Comment(
        f" Identified reflected motor inertia Ia [kg·m²]: {pairs} "
        f"(per-joint feed-forward term Ia·q̈ — add to your controller / "
        f"pinocchio model.armature; a URDF cannot represent it) "
    )


def _build_urdf_string(phi: np.ndarray, args, ia=None) -> str:
...
def _write_urdf(phi: np.ndarray, args, ia=None) -> None:
