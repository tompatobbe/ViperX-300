...
"""
═══════════════════════════════════════════════════════════════════════════════
  ViperX 300s — Trossen DH Table Visualizer
  This file displays the ViperX 300 using the original Trossen robotics DH table.

  Use this file to compare the vendor DH frame construction with any custom model.

  Run:
      python3 Visualizer_trossen.py --table trossen
      python3 Visualizer_trossen.py --table simplified
...
#  Columns: [ θ_offset(°), d(mm), a(mm), α(°) ]
# ─────────────────────────────────────────────────────────────────────────────

TROSSEN_DH_TABLE = np.array([
    #   θ_off(°)    d(mm)    a(mm)   α(°)   Joint name
...
SIMPLIFIED_DH_TABLE = np.array([
    #   α(°)    a(mm)   d(mm)    θ_off(°)   Joint name
    [    0.0,    0.0,    0.12675,  0.0   ], # 1  Waist
    [    3*np.pi/2,  0.0,  0.0,    0.0     ],  # 2  Shoulder
    [    0.0,     0.0,  200.0,   90.0     ],  # 3  Elbow
    [    0.0,    50.0,     0.0,    0.0   ], # 4  Wrist Rotate
    [    0.0,     0.0,     0.0,   90.0   ], # 5  Wrist Pitch
    [    0.0,    50.0,     0.0,  -90.0   ], # 6  Wrist Roll
])

DH_TABLE = np.array([
    #   α(°)    a(mm)   d(mm)    θ_off(°)   Joint name
    [   0.0,   0.0,    L_1,   0.0   ],  # 1  Waist
    [  -90,     0.0,    0.0,    -0.437*np.pi], # 2  Shoulder
...
class DHVisualizer:
    ...
    def __init__(self, dh_table: np.ndarray, table_name: str):
        ...
        self.fig.canvas.manager.set_window_title(
            f"ViperX 300s  ·  {table_name} DH Visualizer")

        self.ax3d = self.fig.add_axes([0.01, 0.02, 0.55, 0.95], projection='3d')
    ...
    def _build_sliders(self):
        ...
        for i in range(N_JOINTS):
            ...
            ax_s = self.fig.add_axes([0.60, y0 - i * dy, 0.355, 0.025], facecolor="#1e1e40")
            s = Slider(ax_s,
    ...
    def _draw_legend(self):
        ...
        entries = [("X axis", "#ff4444"), ("Y axis", "#44ff88"), ("Z axis", "#4488ff")]
    ...
    def _draw_table(self):
        ...
        ax.text(0.5, 0.97, f"DH Parameter Table  ·  {self.table_name} Convention",
                ha='center', va='top', color='white',
                fontsize=9.5, fontweight='bold', transform=ax.transAxes)

        headers = ["Joint", "θ_offset (°)", "d (mm)", "a (mm)", "α (°)", "θ_joint (°)"]
    ...
    def _draw_3d(self, frames):
        ...
        ax.set_title(f"ViperX 300s  ·  {self.table_name} Frames in 3D Space",
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="ViperX 300 DH visualizer using different DH tables.")
    parser.add_argument("--table", choices=["trossen", "simplified"],
                        default="trossen",
                        help="Choose which DH table to visualize.")
    return parser.parse_args()


if __name__ == "__main__":
    args = parse_args()
    dh_table = TROSSEN_DH_TABLE if args.table == "trossen" else SIMPLIFIED_DH_TABLE
    table_name = "Trossen" if args.table == "trossen" else "Simplified"

    print(__doc__)
    print(f"Launching {table_name} DH visualizer …  (close the window to exit)\n")
    try:
        app = DHVisualizer(dh_table, table_name)
        app.run()
